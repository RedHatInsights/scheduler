...
func (s *ExportPollerService) scanAndProcess(ctx context.Context) {
	...
	// Database-only mode: use FOR UPDATE SKIP LOCKED to atomically claim runs
	// This replaces Redis distributed locking with database-native coordination
	if s.lock == nil {
		// Check if repository supports FetchInFlightExportRuns (database-only mode)
		type DatabasePollingRepo interface {
			FetchInFlightExportRuns(ctx context.Context, limit int, maxAge time.Duration) ([]domain.JobRun, error)
		}

		if dbRepo, ok := s.runRepo.(DatabasePollingRepo); ok {
			exportRuns, err = dbRepo.FetchInFlightExportRuns(ctx, s.maxConcurrentPolls, s.maxAge)
			if err != nil {
				s.logger.Error("Failed to fetch in-flight exports", slog.Any("error", err))
				return
			}
		} else {
			s.logger.Error("Database-only mode requires FetchInFlightExportRuns method in repository")
			return
		}
	} else {
		// Redis mode: use FindByStatus with distributed locking
		type StatusQueryRepo interface {
			FindByStatus(ctx context.Context, status domain.JobRunStatus) ([]domain.JobRun, error)
		}

		if statusRepo, ok := s.runRepo.(StatusQueryRepo); ok {
			runs, err := statusRepo.FindByStatus(ctx, domain.RunStatusRunning)
			if err != nil {
			...
			s.logger.Error("Redis mode requires FindByStatus method in repository")
			return
		}
	}
