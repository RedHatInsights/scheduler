...
func (s *DefaultSchedulingService) ShouldRun(job domain.Job, currentTime time.Time) bool {
	...
	case domain.Schedule10Minutes:
		return currentTime.Sub(*job.LastRunAt) >= 10*time.Minute
	case domain.Schedule1Hour:
		return currentTime.Sub(*job.LastRunAt) >= time.Hour
	case domain.Schedule1Day:
		return currentTime.Sub(*job.LastRunAt) >= 24*time.Hour
	case domain.Schedule1Month:
		return s.shouldRunMonthly(job.LastRunAt, &currentTime)
	default:
		return false
	}
}
